  ...
  useEffect(() => {
    ...
    class DataEngine {
        ...
        async fetchYfinancePrice(symbol: string, timeframe: string) {
            const url = `http://127.0.0.1:5004/api/forex-data?pair=${symbol}&timeframe=${timeframe}`;
            const data = await fetchWithRetry(url);
            if (data && Array.isArray(data) && data.length > 0) {
                return { history: data, provider: 'yfinance' };
            }
            throw new Error(`Invalid or empty historical data from yfinance for ${symbol}`);
        }

        async fetchLatestYfinancePrice(symbol: string) {
            const url = `http://127.0.0.1:5004/api/forex-price?pair=${symbol}`;
            const data = await fetchWithRetry(url);
            if (data && data.price) {
                return { price: data.price, provider: 'yfinance' };
            }
            throw new Error(`Invalid or empty price data from yfinance for ${symbol}`);
        }

        async getPrice(symbol: string, timeframe: string) {
            const tryLatestYfinance = async () => {
                log(`📡 Routing ${symbol} to yfinance for latest price...`, 'info');
                const result = await this.fetchLatestYfinancePrice(symbol);
                return result;
            };

            const tryYfinance = async () => {
                log(`📡 Routing ${symbol} to yfinance for ${timeframe} data...`, 'info');
                const result = await this.fetchYfinancePrice(symbol, timeframe);
                return result;
            };

            try {
                return await tryLatestYfinance();
            } catch (latestYfinanceError: any) {
                log(`🟡 yfinance latest price failed for ${symbol}: ${latestYfinanceError.message}. Trying historical...`, 'warning');
                try {
                    return await tryYfinance();
                } catch (yfinanceError: any) {
                    log(`❌ yfinance failed for ${symbol}: ${yfinanceError.message}`, 'error');
                    throw yfinanceError;
                }
            }
        }
    ...
    async function fetchAllForexData(symbols: string[], timeframe: string) {
        try {
            const url = `http://127.0.0.1:5007/api/bulk-forex-price?pairs=${symbols.join(',')}`;
            const data = await fetchWithRetry(url);
            ...
            for (const symbol of symbols) {
                if (data[symbol] && data[symbol].price) {
                    ...
                    lastData[symbol] = data[symbol].price;
            ...
            log(`Error fetching bulk Forex data: ${error.message}`, 'error');
            return null;
        }
    }
    ...
    async function performAdvancedSMCAnalysis(symbol: string, historicalData: any[], timeframe: string) {
        try {
            ...
            const currentPriceData = { price: historicalData[historicalData.length - 1].close.toString() };
    ...
    async function startAnalysis(symbols: string[], timeframes: string[]) {
        ...
        const initialFetch = async () => {
            for (const timeframe of timeframes) {
                const url = `http://127.0.0.1:5004/api/bulk-forex-data?pairs=${symbols.join(',')}&timeframe=${timeframe}`;
        ...
        const analyze = async () => {
            const allData = await fetchAllForexData(symbols, timeframes[0]); // Use first timeframe for live price
            if (allData) {
                for (const symbol of symbols) {
                    const currentPriceData = allData[symbol];
                    if (currentPriceData && currentPriceData.length > 0) {
                        const signals = await performAdvancedSMCAnalysis(symbol, currentPriceData, timeframes[0]);
    ...
    (window as any).startBot = () => {
        ...
        if (selectedOptions.includes('ALL')) {
            selectedSymbols = [...markets.stocks.symbols, ...markets.commodities.symbols, ...markets.forexMajors.symbols, ...markets.forexCrosses.symbols];
        } else {
