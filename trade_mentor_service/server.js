...
io.on('connection', (socket) => {
  ...
  socket.on('sendMessage', async ({ tradeId, message }) => {
    ...
    const userMessage = { id: trade.messages.length + 1, text: message, timestamp: new Date().toISOString(), sender: 'user' };
    ...
    try {
      ...
      const aiMessage = { id: trade.messages.length + 1, text: aiText, timestamp: new Date().toISOString(), sender: 'mentor' };
      trade.messages.push(aiMessage);
      io.to(tradeId).emit('newMessage', aiMessage);

    } catch (error) {
      console.error("Error getting AI response:", error);
      const errorMessage = { id: trade.messages.length + 1, text: "Sorry, I'm having trouble analyzing the trade right now.", timestamp: new Date().toISOString(), sender: 'mentor' };
